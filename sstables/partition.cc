...
future<uint64_t> sstables::sstable::data_end_position(uint64_t summary_idx, const io_priority_class& pc) {
    ...
    if (size_t(summary_idx + 1) >= _sc->summary.entries.size()) {
...
sstables::sstable::find_disk_ranges(
    ...
    if (token < partitioner.get_token(key_view(_sc->summary.first_key.value))
            || token > partitioner.get_token(key_view(_sc->summary.last_key.value))) {
        return make_ready_future<disk_read_range>();
    }
    auto summary_idx = adjust_binary_search_index(binary_search(_sc->summary.entries, key, token));
    if (summary_idx < 0) {
        return make_ready_future<disk_read_range>();
    }
