...
static api::timestamp_type get_max_purgeable_timestamp(const column_family& cf,
    ...
    if (auto maintenance_ssts = maintenance_selector.select(dk).sstables; maintenance_ssts.size()) {
        std::move(maintenance_ssts.begin(), maintenance_ssts.end(), std::back_inserter(sstables));
    }

    for (auto&& sst : boost::range::join(std::move(sstables), cf.compacted_undeleted_sstables())) {
