...
private:
    // Default range sstable reader that will only filter mutation that doesn't belong to current shard
    // from shared sstables.
    virtual mutation_reader make_range_sstable_reader(lw_shared_ptr<sstables::sstable_set> ssts) const {
    ...
    flat_mutation_reader setup() {
        ...
        return flat_mutation_reader_from_mutation_reader(_cf.schema(), make_range_sstable_reader(std::move(ssts)), ::streamed_mutation::forwarding::no);
    }
...
public:
    ...
    // Use range sstable reader that makes sure that mutation not belong to current shard will never
    // be filtered out!
    mutation_reader make_range_sstable_reader(lw_shared_ptr<sstables::sstable_set> ssts) const override {
        return ::make_non_filtering_range_sstable_reader(_cf.schema(),
            std::move(ssts),
            query::full_partition_range,
            _cf.schema()->full_slice(),
            service::get_local_compaction_priority(),
