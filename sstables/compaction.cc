...
private:
    ...
    void setup_garbage_collected_sstable(shared_sstable sst) {
        ...
        _cf.add_sstable_and_update_cache(std::move(sst));
    }
...
void garbage_collected_sstable_writer::setup_on_new_sstable_sealed_handler() {
    _on_new_sstable_sealed_observer = _c->add_on_new_sstable_sealed_handler([this] {
        // called when compaction seals a new sstable, to make sure garbage collected data don't disappear
        // when an exhausted sstable is released, by flushing gc sstable being written.
