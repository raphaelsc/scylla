...
static api::timestamp_type get_max_purgeable_timestamp(const column_family& cf,
    ...
    auto all_sstables_range = boost::range::join(sstables, maintenance_sstables);

    for (auto&& sst : boost::range::join(std::move(all_sstables_range), cf.compacted_undeleted_sstables())) {
