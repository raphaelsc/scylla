...
get_fully_expired_sstables(column_family& cfs, std::vector<sstables::shared_sstable>& compacting, int32_t gc_before) {
    logger.debug("Checking droppable sstables in {}.{}", cfs.schema()->ks_name(), cfs.schema()->cf_name());
    ...
    auto uncompacting_sstables = get_uncompacting_sstables(cfs, compacting);
    // Get list of uncompacting sstables that overlap the ones being compacted.
    std::vector<sstables::shared_sstable> overlapping = leveled_manifest::overlapping(*cfs.schema(), compacting, uncompacting_sstables);
