...
public:
    ...
    virtual bool has_incremental_compaction_enabled() const {
        return false;
    }
...
public:
    ...
    virtual bool has_incremental_compaction_enabled() const override {
...
compact_sstables(sstables::compaction_descriptor descriptor, column_family& cf, std::function<shared_sstable()> creator, replacer_fn replacer, bool cleanup) {
    ...
    if (c->has_incremental_compaction_enabled()) {
        return compaction::run(std::move(c), c->get_garbage_collected_sstable_writer());
    }
