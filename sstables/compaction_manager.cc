...
static inline int calculate_weight(uint64_t total_size) {
    // FIXME: should we really use a hardcoded base to determine weight of a
    // compaction job?
    return int(std::log(total_size) / std::log(4));
}
...
void compaction_manager::trim_to_compact(column_family* cf, std::vector<sstables::shared_sstable>& sstables) {
    ...
    uint64_t total_size = get_total_size(sstables);
    int min_threshold = cf->schema()->min_compaction_threshold();

    while (sstables.size() > size_t(min_threshold)) {
        int weight = calculate_weight(total_size);
        if (s.count(weight)) {
            ...
            return seastar::with_gate(task->compaction_gate, [this, task] {
                ...
                if (task->cleanup) {
                    ...
                    if (!try_to_register_weight(&cf, weight)) {
                        // Stop this task if there is an ongoing compaction for
                        // this column family with exactly the same weight.
                        task->stopping = true;
                        _stats.pending_tasks--;
