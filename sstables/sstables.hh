...
public:
    ...
    future<> load(sstables::foreign_sstable_open_info info);
...
struct foreign_sstable_open_info {
    foreign_sstable_open_info(std::vector<shard_id> owners, foreign_ptr<lw_shared_ptr<sstable::shareable_components>> components,
            seastar::file_handle data, seastar::file_handle index)
        : owners(std::move(owners)), components(std::move(components)), data(std::move(data)), index(std::move(index)) {
    }
...
struct sstable_open_info {
    sstable_open_info(std::vector<shard_id> owners, lw_shared_ptr<sstable::shareable_components> components,
            seastar::file_handle data, seastar::file_handle index)
        : owners(std::move(owners)), components(std::move(components)), data(std::move(data)), index(std::move(index)) {
    }
    std::vector<shard_id> owners;
