...
public:
    ...
    io_check_error_handler& error_handler() const {
        return const_cast<io_check_error_handler&>(_error_handler);
    }
    template<typename Func, typename... Args>
    auto sstable_write_io_check(Func&& func, Args&&... args) const {
        return do_io_check(error_handler(), sstable_write_error, func, std::forward<Args>(args)...);
    }
private:
    sstable(size_t wbuffer_size, schema_ptr schema, sstring dir, int64_t generation, version_types v, format_types f,
            gc_clock::time_point now = gc_clock::now(), io_check_error_handler error_handler = default_io_check_error_handler())
