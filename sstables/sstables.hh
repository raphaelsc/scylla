...
public:
    ...
    uint64_t get_estimated_key_count() const {
        return ((uint64_t)_sc->summary.header.size_at_full_sampling + 1) *
                _sc->summary.header.min_index_interval;
    }
...
private:
    ...
    foreign_ptr<lw_shared_ptr<shareable_components>> _sc = make_foreign(make_lw_shared<shareable_components>());
    ...
    void write_summary(const io_priority_class& pc) {
        write_simple<component_type::Summary>(_sc->summary, pc);
    }
...
public:
    ...
    bool filter_has_key(const key& key) {
        return _sc->filter->is_present(bytes_view(key));
    }

    bool filter_has_key(utils::hashed_key key) {
        return _sc->filter->is_present(key);
    }
    ...
    const compaction_metadata& get_compaction_metadata() const {
        auto entry = _sc->statistics.contents.find(metadata_type::Compaction);
        if (entry == _sc->statistics.contents.end()) {
