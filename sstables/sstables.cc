...
future<summary_entry&> sstable::read_summary_entry(size_t i) {
    ...
    if (i >= (_sc->summary.entries.size())) {
        throw std::out_of_range(sprint("Invalid Summary index: %ld", i));
    }

    return make_ready_future<summary_entry&>(_sc->summary.entries[i]);
}
...
future<> sstable::read_compression(const io_priority_class& pc) {
    ...
    return read_simple<component_type::CompressionInfo>(_sc->compression, pc);
}
...
void sstable::write_compression(const io_priority_class& pc) {
    ...
    write_simple<component_type::CompressionInfo>(_sc->compression, pc);
}

void sstable::validate_min_max_metadata() {
    auto entry = _sc->statistics.contents.find(metadata_type::Stats);
    if (entry == _sc->statistics.contents.end()) {
...
future<> sstable::read_statistics(const io_priority_class& pc) {
    return read_simple<component_type::Statistics>(_sc->statistics, pc);
}

void sstable::write_statistics(const io_priority_class& pc) {
    write_simple<component_type::Statistics>(_sc->statistics, pc);
}
...
future<> sstable::read_summary(const io_priority_class& pc) {
    ...
    return read_toc().then([this, &pc] {
        ...
        if (has_component(sstable::component_type::Summary)) {
            return read_simple<component_type::Summary>(_sc->summary, pc).handle_exception([this, &pc] (auto ep) {
...
size_t components_writer::get_offset() {
    if (_sst.has_component(sstable::component_type::CompressionInfo)) {
        ...
        return _sst._sc->compression.compressed_file_length();
    } else {
        return _out.offset();
    }
}
...
{
    _sst._sc->filter = utils::i_filter::get_filter(estimated_partitions, _schema.bloom_filter_fp_chance());
    _sst._pi_write.desired_block_size = get_config().column_index_size_in_kb() * 1024;

    prepare_summary(_sst._sc->summary, estimated_partitions, _schema.min_index_interval());
...
void components_writer::consume_new_partition(const dht::decorated_key& dk) {
    ...
    maybe_add_summary_entry(_sst._sc->summary, bytes_view(*_partition_key), _index.offset());
    _sst._sc->filter->add(bytes_view(*_partition_key));
...
void components_writer::consume_end_of_stream() {
    seal_summary(_sst._sc->summary, std::move(_first_key), std::move(_last_key)); // what if there is only one partition? what if it is empty?

    _index.close().get();

    if (_sst.has_component(sstable::component_type::CompressionInfo)) {
        _sst._collector.add_compression_ratio(_sst._sc->compression.compressed_file_length(), _sst._sc->compression.uncompressed_file_length());
    }

    _sst.set_first_and_last_keys();
    seal_statistics(_sst._sc->statistics, _sst._collector, dht::global_partitioner().name(), _schema.bloom_filter_fp_chance(),
...
sstable::read_scylla_metadata(const io_priority_class& pc) {
    if (_sc->scylla_metadata) {
        return make_ready_future<>();
    }
    return read_toc().then([this, &pc] {
        _sc->scylla_metadata.emplace();  // engaged optional means we won't try to re-read this again
        if (!has_component(component_type::Scylla)) {
            return make_ready_future<>();
        }
        return read_simple<component_type::Scylla>(*_sc->scylla_metadata, pc);
    });
}
...
sstable::write_scylla_metadata(const io_priority_class& pc) {
    ...
    _sc->scylla_metadata.emplace();
    _sc->scylla_metadata->data.set<scylla_metadata_type::Sharding>(std::move(sm));

    write_simple<component_type::Scylla>(*_sc->scylla_metadata, pc);
}
...
void sstable_writer::prepare_file_writer()
{
    ...
    if (!_compression_enabled) {
        ...
        prepare_compression(_sst._sc->compression, _schema);
        _writer = make_shared<file_writer>(make_compressed_file_output_stream(std::move(_sst._data_file), std::move(options), &_sst._sc->compression));
    }
}
...
void sstable_writer::finish_file_writer()
{
    ...
    if (!_compression_enabled) {
        ...
        write_digest(_sst._write_error_handler, _sst.filename(sstable::component_type::Digest), _sst._sc->compression.full_checksum());
    }
}
    ...
    return open_checked_file_dma(_read_error_handler, filename(component_type::Index), open_flags::ro).then([this, &pc] (file index_file) {
        return do_with(std::move(index_file), [this, &pc] (file index_file) {
            return index_file.size().then([this, &pc, index_file] (auto size) {
                ...
                prepare_summary(_sc->summary, estimated_partitions, _schema->min_index_interval());

                file_input_stream_options options;
                options.buffer_size = sstable_buffer_size;
                options.io_priority_class = pc;
                auto stream = make_file_input_stream(index_file, 0, size, std::move(options));
                return do_with(summary_generator(_sc->summary), [this, &pc, stream = std::move(stream), size] (summary_generator& s) mutable {
                    ...
                    return ctx->consume_input(*ctx).finally([ctx] {
                        ...
                        seal_summary(_sc->summary, std::move(s.first_key), std::move(s.last_key));
                    });
                });
...
uint64_t sstable::data_size() const {
    if (has_component(sstable::component_type::CompressionInfo)) {
        return _sc->compression.data_len;
    }
...
input_stream<char> sstable::data_stream(uint64_t pos, size_t len, const io_priority_class& pc, lw_shared_ptr<file_input_stream_history> history) {
    ...
    if (_sc->compression) {
        return make_compressed_file_input_stream(_data_file, &_sc->compression,
...
void sstable::set_first_and_last_keys() {
    ...
    _first = decorate_key("first", _sc->summary.first_key.value);
    _last = decorate_key("last", _sc->summary.last_key.value);
}
...
double sstable::get_compression_ratio() const {
    if (this->has_component(sstable::component_type::CompressionInfo)) {
        return double(_sc->compression.compressed_file_length()) / _sc->compression.uncompressed_file_length();
    } else {
...
stdx::optional<std::pair<uint64_t, uint64_t>> sstable::get_sample_indexes_for_range(const dht::token_range& range) {
    auto entries_size = _sc->summary.entries.size();
    auto search = [this](bool before, const dht::token& token) {
        ...
        return uint64_t((binary_search(_sc->summary.entries, k, token) + 1) * -1);
    };
    uint64_t left = 0;
    if (range.start()) {
...
std::vector<dht::decorated_key> sstable::get_key_samples(const schema& s, const dht::token_range& range) {
    ...
    if (index_range) {
        for (auto idx = index_range->first; idx < index_range->second; ++idx) {
            auto pkey = _sc->summary.entries[idx].get_key().to_partition_key(s);
...
uint64_t sstable::estimated_keys_for_range(const dht::token_range& range) {
    ...
    uint64_t estimated_keys = sample_key_count * ((downsampling::BASE_SAMPLING_LEVEL * _sc->summary.header.min_index_interval) / _sc->summary.header.sampling_level);
...
sstable::get_shards_for_this_sstable() const {
    ...
    const auto* sm = _sc->scylla_metadata
            ? _sc->scylla_metadata->data.get<scylla_metadata_type::Sharding, sharding_metadata>()
            : nullptr;
    if (!sm) {
