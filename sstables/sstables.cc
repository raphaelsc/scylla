...
extern bool ignore_sstable_if_irrelevant(const schema& s, sstables::sstable& sstable);
...
future<bool>
sstable::ignore_if_irrelevant(const schema& s, sstring ks, sstring cf, sstring dir, int64_t generation, version_types v, format_types f) {
    ...
    return std::move(fut).then([sst = std::move(sst), &s] () mutable {
        return make_ready_future<bool>(::ignore_sstable_if_irrelevant(s, *sst));
    });
}
