...
future<file> new_sstable_component_file(io_error_handler& error_handler, sstring name, open_flags flags) {
...
future<>
fsync_directory(io_error_handler& error_handler, sstring fname) {
    return ::sstable_io_check(error_handler, [&, error_handler] {
...
future<>
remove_by_toc_name(sstring sstable_toc_name, io_error_handler error_handler) {
    return seastar::async([sstable_toc_name, error_handler = std::move(error_handler)] () mutable {
        ...
        if (sstable_io_check(error_handler, file_exists, sstable_toc_name).get0()) {
            ...
            sstable_io_check(error_handler, fsync_directory, error_handler, dir).get();
        } else {
...
sstable::mark_for_deletion_on_disk() {
    ...
    return smp::submit_to(shard, [this, toc_name] {
        ...
        return seastar::async([this, toc_name] {
            ...
            sstable_write_io_check(fsync_directory, _write_error_handler, dir).get();
        }).finally([toc_name] {
...
sstable::remove_sstable_with_temp_toc(sstring ks, sstring cf, sstring dir, int64_t generation, version_types v, format_types f) {
    return seastar::async([ks, cf, dir, generation, v, f] {
        io_error_handler error_handler = default_io_error_handler(sstable_write_error);
