...
static future<file> open_sstable_component_file(const io_error_handler& error_handler, Params&&... params) {
    if (service::get_local_storage_service().sstable_data_integrity_check()) {
        return open_integrity_checked_file_dma(std::forward<Params>(params)...);
    } else {
        return open_checked_file_dma(error_handler, std::forward<Params>(params)...);
    }
}

future<file> new_sstable_component_file(const io_error_handler& error_handler, sstring name, open_flags flags) {
    return open_sstable_component_file(error_handler, name, flags).handle_exception([name] (auto ep) {
