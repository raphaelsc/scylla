...
sstable_writer::sstable_writer(sstable& sst, const schema& s, uint64_t estimated_partitions, uint64_t max_sstable_size,
                               bool backup, bool leave_unsealed, const io_priority_class& pc, shard_id shard)
    : _sst(sst)
    , _schema(s)
    , _pc(pc)
    , _backup(backup)
...
sstable_writer sstable::get_writer(const schema& s, uint64_t estimated_partitions, uint64_t max_sstable_size,
                                   bool backup, const io_priority_class& pc, bool leave_unsealed, shard_id shard)
{
    return sstable_writer(*this, s, estimated_partitions, max_sstable_size, backup, leave_unsealed, pc, shard);
}
