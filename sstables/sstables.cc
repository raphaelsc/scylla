...
future<file> new_sstable_component_file(io_check_error_handler& error_handler, disk_error_signal_type& signal, sstring name, open_flags flags) {
    return open_checked_file_dma(signal, name, flags, error_handler).handle_exception([name] (auto ep) {
...
future<file> new_sstable_component_file(io_check_error_handler& error_handler, disk_error_signal_type& signal, sstring name, open_flags flags,
    ...
    return open_checked_file_dma(signal, name, flags, options, error_handler).handle_exception([name] (auto ep) {
        sstlog.error("Could not create SSTable component {}. Found exception: {}", name, ep);
        return make_exception_future<file>(ep);
    });
}
...
future<> sstable::read_toc() {
    ...
    return open_checked_file_dma(sstable_read_error, file_path, open_flags::ro, error_handler()).then([this, file_path] (file f) {
...
void sstable::write_toc(const io_priority_class& pc) {
    ...
    file f = new_sstable_component_file(error_handler(), sstable_write_error, file_path, open_flags::wo | open_flags::create | open_flags::exclusive).get0();
    ...
    // the disk.
    file dir_f = open_checked_directory(sstable_write_error, _dir, error_handler()).get0();
...
future<> sstable::seal_sstable() {
    ...
    return open_checked_directory(sstable_write_error, _dir, error_handler()).then([this] (file dir_f) {
...
void write_crc(io_check_error_handler& error_handler, const sstring file_path, checksum& c) {
    sstlog.debug("Writing CRC file {} ", file_path);

    auto oflags = open_flags::wo | open_flags::create | open_flags::exclusive;
    file f = new_sstable_component_file(error_handler, sstable_write_error, file_path, oflags).get0();
...
void write_digest(io_check_error_handler& error_handler, const sstring file_path, uint32_t full_checksum) {
    sstlog.debug("Writing Digest file {} ", file_path);

    auto oflags = open_flags::wo | open_flags::create | open_flags::exclusive;
    auto f = new_sstable_component_file(error_handler, sstable_write_error, file_path, oflags).get0();
...
future<> sstable::read_simple(T& component, const io_priority_class& pc) {
    ...
    return open_file_dma(file_path, open_flags::ro).then([this, &component] (file fi) {
        auto f = make_checked_file(error_handler(), sstable_read_error, fi);
...
void sstable::rewrite_statistics(const io_priority_class& pc) {
    ...
    file f = new_sstable_component_file(error_handler(), sstable_write_error, file_path, open_flags::wo | open_flags::create | open_flags::truncate).get0();
...
future<> sstable::open_data() {
    return when_all(open_checked_file_dma(sstable_read_error, filename(component_type::Index), open_flags::ro, error_handler()),
                    open_checked_file_dma(sstable_read_error, filename(component_type::Data), open_flags::ro, error_handler()))
...
future<> sstable::create_data() {
    ...
    return when_all(new_sstable_component_file(error_handler(), sstable_write_error, filename(component_type::Index), oflags, opt),
                    new_sstable_component_file(error_handler(), sstable_write_error, filename(component_type::Data), oflags, opt)).then([this] (auto files) {
...
void sstable_writer::finish_file_writer()
{
    ...
    if (!_compression_enabled) {
        ...
        write_digest(_sst.error_handler(), _sst.filename(sstable::component_type::Digest), chksum_wr->full_checksum());
        write_crc(_sst.error_handler(), _sst.filename(sstable::component_type::CRC), chksum_wr->finalize_checksum());
    } else {
        write_digest(_sst.error_handler(), _sst.filename(sstable::component_type::Digest), _sst._compression.full_checksum());
    }
}
...
future<> sstable::generate_summary(const io_priority_class& pc) {
    ...
    return open_checked_file_dma(sstable_read_error, filename(component_type::Index), open_flags::ro, error_handler()).then([this, &pc] (file index_file) {
...
future<>
fsync_directory(io_check_error_handler& error_handler, sstring fname) {
    return ::sstable_write_io_check(error_handler, [&, error_handler] {
        return open_checked_directory(sstable_write_error ,dirname(fname), std::move(error_handler)).then([] (file f) {
...
future<>
remove_by_toc_name(sstring sstable_toc_name, io_check_error_handler error_handler) {
    return seastar::async([sstable_toc_name, error_handler = std::move(error_handler)] () mutable {
        ...
        if (sstable_write_io_check(error_handler, file_exists, sstable_toc_name).get0()) {
            dir = dirname(sstable_toc_name);
            sstable_write_io_check(error_handler, rename_file, sstable_toc_name, new_toc_name).get();
            sstable_write_io_check(error_handler, fsync_directory, error_handler, dir).get();
        } else {
            dir = dirname(new_toc_name);
        }

        auto toc_file = open_checked_file_dma(sstable_read_error, new_toc_name, open_flags::ro, error_handler).get0();
        ...
        parallel_for_each(components, [prefix, error_handler] (sstring component) mutable {
            ...
            return ::sstable_write_io_check(error_handler, remove_file, prefix + component).then_wrapped([fname = std::move(fname)] (future<> f) {
        ...
        sstable_write_io_check(error_handler, fsync_directory, error_handler, dir).get();
        sstable_write_io_check(error_handler, remove_file, new_toc_name).get();
    });
}
...
sstable::mark_for_deletion_on_disk() {
    ...
    return smp::submit_to(shard, [this, toc_name] {
        ...
        return seastar::async([this, toc_name] {
            ...
            auto toc_file = open_checked_file_dma(sstable_read_error, toc_name, open_flags::ro, error_handler()).get0();
            ...
            sstable_write_io_check(fsync_directory, error_handler(), dir).get();
        }).finally([toc_name] {
...
sstable::remove_sstable_with_temp_toc(sstring ks, sstring cf, sstring dir, int64_t generation, version_types v, format_types f) {
    return seastar::async([ks, cf, dir, generation, v, f] {
        io_check_error_handler error_handler = default_io_check_error_handler();
        auto toc = ::sstable_write_io_check(error_handler, file_exists, filename(dir, ks, cf, v, generation, f, component_type::TOC)).get0();
        // assert that toc doesn't exist for sstable with temporary toc.
        assert(toc == false);

        auto tmptoc = ::sstable_write_io_check(error_handler, file_exists, filename(dir, ks, cf, v, generation, f, component_type::TemporaryTOC)).get0();
        ...
        for (auto& entry : sstable::_component_map) {
            ...
            auto exists = ::sstable_write_io_check(error_handler, file_exists, file_path).get0();
            if (!exists) {
                continue;
            }
            ::sstable_write_io_check(error_handler, remove_file, file_path).get();
        }
        ::sstable_write_io_check(error_handler, fsync_directory, error_handler, dir).get();
        // Removing temporary
        ::sstable_write_io_check(error_handler, remove_file, filename(dir, ks, cf, v, generation, f, component_type::TemporaryTOC)).get();
