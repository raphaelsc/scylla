...
public:
    virtual input_stream<char> open_at(uint64_t pos) override {
        if (pos > _file_size) {
            throw std::runtime_error("file_random_access_reader: Tried to open file beyond its size");
        }
        auto len = _file_size - pos;
        file_input_stream_options options;
        options.buffer_size = std::min(align_up(len, _file.disk_read_dma_alignment()), _buffer_size);
...
future<> sstable::load() {
    return read_toc().then([this] {
        return seastar::when_all_succeed(read_statistics(default_priority_class()),
