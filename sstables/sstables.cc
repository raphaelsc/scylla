...
void sstable::set_clustering_components_ranges() {
    ...
    for (auto i = 0U; i < s; i++) {
        auto r = range<bytes_view>({{ min_column_names[i].value, true }}, {{ max_column_names[i].value, true }});
        _clustering_components_ranges.push_back(std::move(r));
    }
}

const std::vector<range<bytes_view>>& sstable::clustering_components_ranges() const {
