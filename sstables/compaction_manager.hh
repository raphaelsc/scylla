...
public:
    ...
    // If weight of sstables is taken, we will trim list until its new weight
    // is not taken or its size is equal to minimum threshold.
    // That's important to prevent small sstables from falling behind.
    void trim_to_compact(column_family* cf, std::vector<sstables::shared_sstable>& sstables);
