...
std::unique_ptr<sstable_set_impl> compound_sstable_set::clone() const {
    return std::make_unique<compound_sstable_set>(*this);
}
...
lw_shared_ptr<sstable_list> compound_sstable_set::all() const {
    ...
    for (auto& set : _sets) {
        auto ssts = set->all();
        // optimize for common case where only main set contains sstables, but maintenance one is empty for most of the time.
        if (!ssts->empty()) {
            if (ret->empty()) {
                ret = std::move(ssts);
            } else {
                // copy list if we need to extend it as we cannot modify a list which may be referenced by a sstable_set.
