...
class bag_sstable_set::incremental_selector : public incremental_selector_impl {
    std::unique_ptr<const bag_sstable_set> _set;
public:
    incremental_selector(std::unique_ptr<const bag_sstable_set> set)
        : _set(std::move(set)) {
    }
...
std::unique_ptr<incremental_selector_impl> bag_sstable_set::make_incremental_selector() const {
    return std::make_unique<incremental_selector>(std::make_unique<const bag_sstable_set>(*this));
}
...
class partitioned_sstable_set::incremental_selector : public incremental_selector_impl {
    std::unique_ptr<const partitioned_sstable_set> _set;
    schema_ptr _schema;
    map_iterator _it;
...
public:
    incremental_selector(std::unique_ptr<const partitioned_sstable_set> set, schema_ptr schema)
        : _set(std::move(set))
        , _schema(std::move(schema))
        , _it(_set->_sstables.begin())
        , _end(_set->_sstables.end()) {
    }
    virtual std::pair<nonwrapping_range<dht::token>, std::vector<shared_sstable>> select(const dht::token& token) override {
...
std::unique_ptr<incremental_selector_impl> partitioned_sstable_set::make_incremental_selector() const {
    return std::make_unique<incremental_selector>(std::make_unique<const partitioned_sstable_set>(*this), _schema);
}
