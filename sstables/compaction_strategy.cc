...
protected:
    ...
    double _tombstone_threshold = DEFAULT_TOMBSTONE_THRESHOLD;
    long _tombstone_compaction_interval = DEFAULT_TOMBSTONE_COMPACTION_INTERVAL;
public:
...
protected:
    ...
    compaction_strategy_impl(const std::map<sstring, sstring>& options) {
        ...
        _tombstone_compaction_interval = property_definitions::to_long(TOMBSTONE_COMPACTION_INTERVAL_OPTION, tmp_value, DEFAULT_TOMBSTONE_COMPACTION_INTERVAL);
...
public:
    ...
    bool worth_dropping_tombstones(const shared_sstable& sst, gc_clock::time_point gc_before) {
        ...
        if (db_clock::now()-db_clock::duration(std::chrono::seconds(_tombstone_compaction_interval)) < sst->data_file_write_time()) {
            return false;
        }
...
compaction_descriptor size_tiered_compaction_strategy::get_sstables_for_compaction(column_family& cfs, std::vector<sstables::shared_sstable> candidates) {
    ...
    for (auto rit = buckets.rbegin(); rit != buckets.rend(); ++rit) {
        ...
        std::sort(sstables.begin(), sstables.end(), [] (auto& i, auto& j) {
            return i->get_stats_metadata().min_timestamp < j->get_stats_metadata().min_timestamp;
        });
        return sstables::compaction_descriptor({ sstables.front() });
    }
    return sstables::compaction_descriptor();
}
...
compaction_descriptor leveled_compaction_strategy::get_sstables_for_compaction(column_family& cfs, std::vector<sstables::shared_sstable> candidates) {
    ...
    for (auto level = manifest.get_level_count(); level >= 0; level--) {
        ...
        return sstables::compaction_descriptor({ sstables.back() }, sstables.back()->get_sstable_level());
    }
}
...
public:
    ...
    virtual compaction_descriptor get_sstables_for_compaction(column_family& cfs, std::vector<sstables::shared_sstable> candidates) override {
        ...
        return sstables::compaction_descriptor({ candidates.front() });
    }
