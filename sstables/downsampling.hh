...
public:
    ...
    static const std::vector<int> _empty_sample_pattern; // needed because get_sampling_pattern returns a const ref.

    static std::array<std::vector<int>, BASE_SAMPLING_LEVEL> _sample_pattern_cache;

    static std::array<std::vector<int>, BASE_SAMPLING_LEVEL> _original_index_cache;
    ...
    static const std::vector<int>& get_sampling_pattern(int sampling_level) {
        ...
        if (sampling_level <= 1) {
            assert(_empty_sample_pattern.empty());
            return _empty_sample_pattern;
        }
        ...
        std::vector<int> ordering = get_sampling_pattern(sampling_level/2);
    ...
    static const std::vector<int>& get_original_indexes(int sampling_level) {
        ...
        for (int j = 0; j < BASE_SAMPLING_LEVEL; j++) {
            auto it = std::find(pattern.begin(), pattern.begin() + (BASE_SAMPLING_LEVEL - sampling_level), j);
            if (it == pattern.begin() + (BASE_SAMPLING_LEVEL - sampling_level)) {
