...
public:
    ...
    // std::vector that contains zero as its unique element. Needed because get_sampling_pattern returns a const ref.
    static const std::vector<int> _sample_pattern_with_zero;
    ...
    static const std::vector<int>& get_sampling_pattern(int sampling_level) {
        ...
        if (sampling_level <= 1) {
            assert(_sample_pattern_with_zero.size() == 1);
            assert(_sample_pattern_with_zero[0] == 0);
            return _sample_pattern_with_zero;
        }
