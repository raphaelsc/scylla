...
public:
    ...
    void update_min_column_names(const schema& s, std::vector<bytes>&& min_column_names) {
        if (min_column_names.size() > 0) {
            column_name_helper::merge_min_components(s, _min_column_names, std::move(min_column_names));
        }
    }

    void update_max_column_names(const schema& s, std::vector<bytes>&& max_column_names) {
        if (max_column_names.size() > 0) {
            column_name_helper::merge_max_components(s, _max_column_names, std::move(max_column_names));
        }
    }
    ...
    void update(const schema& s, column_stats&& stats) {
        ...
        update_min_column_names(s, std::move(stats.min_column_names));
        update_max_column_names(s, std::move(stats.max_column_names));
