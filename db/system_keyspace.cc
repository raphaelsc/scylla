...
void make(database& db, bool durable, bool volatile_testing_only) {
    ...
    kscfg.read_concurreny_config.sem = &db.system_keyspace_read_concurrency_sem();
    kscfg.read_concurreny_config.timeout = {};
    kscfg.read_concurreny_config.max_queue_length = std::numeric_limits<size_t>::max();
