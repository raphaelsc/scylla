...
future<std::vector<compaction_history_entry>> get_compaction_history()
{
    ...
    return execute_cql(req, COMPACTION_HISTORY).then([] (::shared_ptr<cql3::untyped_result_set> msg) {
        ...
        return do_for_each(*msg, [history] (auto& row) {
            ...
            entry.compacted_at = row.template get_as<int64_t>("compacted_at");
            entry.bytes_in = row.template get_as<int64_t>("bytes_in");
            entry.bytes_out = row.template get_as<int64_t>("bytes_out");
            if (row.has("rows_merged")) {
                entry.rows_merged = row.template get_map<int32_t, int64_t>("rows_merged");
            }
            history->push_back(std::move(entry));

            return make_ready_future<>();
        }).then([history, msg] () mutable {
