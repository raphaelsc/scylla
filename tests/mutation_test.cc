...
template <typename ContainerMeta>
typename ContainerMeta::container merge_container(
        ContainerMeta meta,
        typename ContainerMeta::container a,
        typename ContainerMeta::container b,
        std::function<int(const typename ContainerMeta::container::value_type&, const typename ContainerMeta::container::value_type&)> tri_cmp,
        std::function<typename ContainerMeta::container::value_type(typename ContainerMeta::container::value_type, typename ContainerMeta::container::value_type)> merge_func) {
    auto merged = meta.construct();
    auto it_a = a.begin();
    auto a_end = a.end();
    auto it_b = b.begin();
    auto b_end = b.end();
    ...
    while (it_a != a_end && it_b != b_end) {
        ...
        if (res < 0) {
            meta.emplace(merged, std::move(*it_a++));
        } else if (res == 0) {
            meta.emplace(merged, merge_func(std::move(*it_a++), std::move(*it_b++)));
        } else { // res > 0
            meta.emplace(merged, std::move(*it_b++));
        }
    }
...
std::map<position_in_partition, clustering_row_summary, position_in_partition::less_compare> merge(
    ...
    struct container_meta {
        using container = std::map<position_in_partition, clustering_row_summary, position_in_partition::less_compare>;
        ...
        static void emplace(container& c, typename container::value_type&& e) {
...
std::vector<partition_summary> merge(const schema& s, std::vector<partition_summary> a, std::vector<partition_summary> b) {
    struct container_meta {
        using container = std::vector<partition_summary>;
...
void run_compaction_data_stream_split(const schema& schema, gc_clock::time_point query_time, const std::vector<mutation>& mutations) {
...
SEASTAR_THREAD_TEST_CASE(test_compaction_data_stream_split) {
    ...
    // Completely random data
    {
        ...
        const auto mutations = tests::generate_random_mutations(random_schema).get0();
        run_compaction_data_stream_split(schema, gc_clock::now(), mutations);
    }

    // All data is purged
    {
        tlog.info("All data is purged");
        const auto query_time = gc_clock::now() + (schema.gc_grace_seconds() * 2);
        ...
        const auto mutations = tests::generate_random_mutations(random_schema, ts_gen).get0();
        run_compaction_data_stream_split(schema, query_time, mutations);
    }
