    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        public:
            explicit compacting_sstable_writer_test(const schema_ptr& s, shared_sstable& sst)
                : _sst(sst),
                  _writer(sst->get_writer(*s, 1 /* FIXME: better estimation, this is fine for testing though */,
                                          sstable_writer_config{}, encoding_stats{}, service::get_local_compaction_priority())) {}

            void consume_new_partition(const dht::decorated_key& dk) { _writer.consume_new_partition(dk); }
            void consume(tombstone t) { std::cout << _sst->generation() << ": tombstone\n"; _writer.consume(t); }
            stop_iteration consume(static_row&& sr, tombstone, bool) { std::cout << _sst->generation() << ": srow\n"; return _writer.consume(std::move(sr)); }
            stop_iteration consume(clustering_row&& cr, row_tombstone tomb, bool) {
                std::cout << _sst->generation() << ": crow\n";
                std::cout << cr.tomb() << std::endl;
                std::cout << clustering_row::printer(*(_sst->get_schema()), cr) << std::endl;
                std::cout << "row tomb" << tomb.tomb() << std::endl;
                return _writer.consume(std::move(cr));
            }
            stop_iteration consume(range_tombstone&& rt) { std::cout << _sst->generation() << ": range tombstone\n"; return _writer.consume(std::move(rt)); }
        ...
        auto compact = [&] (std::vector<shared_sstable> all) -> std::pair<shared_sstable, shared_sstable> {
            ...
            auto filter_func = [] (const dht::decorated_key& dk) {
                return true;
            };
            ...
            auto cfc = make_stable_flattened_mutations_consumer<compact_for_compaction<compacting_sstable_writer_test, compacting_sstable_writer_test>>(
                *s, gc_now, std::move(cr), max_purgeable_func, std::move(purged_cr));
            ...
            r.consume_in_thread(std::move(cfc), filter_func, db::no_timeout);
        ...
        auto assert_that_produces_purged_tombstone = [&] (auto& sst, partition_key& key, tombstone tomb) {
            ...
            read_mutation_from_flat_mutation_reader(*reader, db::no_timeout).then([reader, s, &key, is_tombstone_purgeable, &tomb] (mutation_opt m) {
                BOOST_REQUIRE(m);
                BOOST_REQUIRE(m->key().equal(*s, key));
                std::cout << mutation_partition::printer(*s, m->partition()) << std::endl;
                auto& rows = m->partition().clustered_rows();
                for (const auto& entry : rows) {
                    const auto& r = entry.row();
                    std::cout << "crow tomb: " << r.deleted_at().tomb() << std::endl;
                }
                BOOST_REQUIRE_EQUAL(rows.calculate_size(), 1);
                std::cout << m->partition().partition_tombstone() << tomb << std::endl;
