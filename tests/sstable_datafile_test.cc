    ...
    return test_setup::do_with_test_directory([] {
        ...
        return sst->write_components(*mt).then([s, sst] {
            ...
            return sstables::compact_sstables(std::move(sstables), *cf, creator,
                    std::numeric_limits<uint64_t>::max(), 0, true).then([s, key, new_tables] {
...
SEASTAR_TEST_CASE(cf_cleanup_test) {
    auto s = make_lw_shared(schema({}, some_keyspace, some_column_family,
        {{"p1", utf8_type}}, {{"c1", utf8_type}}, {{"r1", int32_type}}, {}, utf8_type));
    ...
    return do_for_each(*generations, [cf, s, tmp, this_shard_keys] (unsigned long generation) {
        auto mt = make_lw_shared<memtable>(s);
        const column_definition& r1_col = *s->get_column_definition("r1");
