...
SEASTAR_TEST_CASE(tombstone_purge_test) {
    ...
    return do_for_each(*memtables, [tmp, gen, sstables, memtables] (lw_shared_ptr<memtable>& mt) {
        ...
        return (*reader)().then([reader] (mutation_opt m) {
            BOOST_REQUIRE(m);
            BOOST_REQUIRE(to_bytes(m->key().representation()) == to_bytes("beta"));
            return (*reader)();
        }).then([reader] (mutation_opt m) {
            BOOST_REQUIRE(m);
            BOOST_REQUIRE(to_bytes(m->key().representation()) == to_bytes("alpha"));
            return (*reader)();
        }).then([reader] (mutation_opt m) {
            BOOST_REQUIRE(!m);
        });
        ...
        auto reader = make_lw_shared(sstable_reader(sst, s));
        return (*reader)().then([reader, tomb] (mutation_opt m) {
            BOOST_REQUIRE(m);
            BOOST_REQUIRE(to_bytes(m->key().representation()) == to_bytes("alpha"));
        ...
        return sstables::compact_sstables(*sstables, *cf, create, std::numeric_limits<uint64_t>::max(), 0).then([s, tmp, sstables, cf, cm] {
            return open_sstable(tmp->path, 3).then([s] (shared_sstable sst) {
                ...
                return (*reader)().then([reader, s] (mutation_opt m) {
                    BOOST_REQUIRE(m);
                    BOOST_REQUIRE(m->key().representation() == bytes("beta"));
                    BOOST_REQUIRE(!m->partition().partition_tombstone());
                    return (*reader)();
                }).then([reader] (mutation_opt m) {
                    BOOST_REQUIRE(!m);
