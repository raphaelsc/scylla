...
public:
    ...
    static void add_column_family(database& db, schema_ptr& schema, column_family::config cfg) {
        ...
        lw_shared_ptr<column_family> cf = make_lw_shared<column_family>(schema, std::move(cfg), column_family::no_commitlog(), db._compaction_manager, *cl_stats);
        auto uuid = schema->id();
...
static schema_ptr get_schema(const utils::UUID& uuid) {
...
void run_sstable_resharding_test() {
    ...
    db.invoke_on_all([uuid, datadir = tmp->path] (database& db) {
        dht::set_global_partitioner(to_sstring("org.apache.cassandra.dht.Murmur3Partitioner"));
        auto s = get_schema(uuid);
