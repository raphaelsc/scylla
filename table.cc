...
table::make_streaming_reader(schema_ptr s,
    ...
    auto source = mutation_source([this] (schema_ptr s, reader_permit permit, const dht::partition_range& range, const query::partition_slice& slice,
        ...
        readers.emplace_back(make_sstable_reader(s, permit, _sstables, range, slice, pc, trace_state, fwd, fwd_mr));
        return make_restricted_combined_reader(std::move(s), std::move(permit), std::move(readers),
                                               range, slice, pc, trace_state, fwd, fwd_mr);
    });
...
flat_mutation_reader table::make_streaming_reader(schema_ptr schema, const dht::partition_range& range,
    ...
    return make_restricted_combined_reader(std::move(schema), std::move(permit), std::move(readers),
                                           range, slice, pc, trace_state, fwd, fwd_mr);
}
...
void
table::rebuild_sstable_lists_for_repair(const std::vector<sstables::shared_sstable>& new_sstables,
                                    const std::vector<sstables::shared_sstable>& old_sstables) {
    auto current_main_sstables = _sstables;
    auto new_main_sstable_list = _compaction_strategy.make_sstable_set(_schema);

    auto current_repair_sstables = _repair_sstables;
    auto new_repair_sstable_list = make_repair_sstable_set();

    // Create main sstable set with new sstables
    for (auto&& tab : boost::range::join(new_sstables, std::move(*current_main_sstables->all()))) {
        new_main_sstable_list.insert(tab);
    }
    // Create repair sstable set without old sstables
    std::unordered_set<sstables::shared_sstable> s(old_sstables.begin(), old_sstables.end());
    for (auto&& tab : *current_repair_sstables->all()) {
        if (!s.contains(tab)) {
            new_repair_sstable_list->insert(tab);
        }
    }
    _sstables = make_lw_shared<sstables::sstable_set>(std::move(new_main_sstable_list));
    _repair_sstables = std::move(new_repair_sstable_list);
}
...
table::on_compaction_completion(sstables::compaction_completion_desc& desc,
    ...
    _cache.invalidate([this, &desc, replacer = std::move(replacer)] () noexcept {
        // FIXME: this is not really noexcept, but we need to provide strong exception guarantees.
        replacer(desc);
...
// TODO: use compaction_manager::submit_offstrategy_compaction() instead?!
future<> table::run_offstrategy_compaction() {
    // FIXME: make sure reshaped sstables aren't compacted more than once.
    // FIXME: probably move run_custom_job() to around compact_sstables, to allow other jobs to run too, or start using compaction_manager::submit_offstrategy_compaction() instead, which may be a good idea.
    // FIXME: probably off-strategy shouldn't block other custom operations that go through compaction manager.
    // FIXME: think about ways to increase its observability through manager
    // FIXME: think about ways to increase its testability for unit tests.
    return _compaction_manager.run_custom_job(this, "off-strategy", [this] () mutable {
        return repeat([this] () mutable {
            auto candidates = boost::copy_range<std::vector<sstables::shared_sstable>>(*_repair_sstables->all());
            // make sure it runs in maintenance mode.
            auto& iop = service::get_local_streaming_priority();
            // TODO: introduce a new reshape mode for integration
            auto desc = _compaction_strategy.get_reshaping_job(std::move(candidates), _schema, iop, sstables::reshape_mode::strict);
            ...
            desc.replacer = [this] (sstables::compaction_completion_desc desc) mutable {
                this->on_compaction_completion(desc, [this] (sstables::compaction_completion_desc& desc) mutable {
                    rebuild_sstable_lists_for_repair(desc.new_sstables, desc.old_sstables);
                });
            };

            return sstables::compact_sstables(std::move(desc), *this).then([] (sstables::compaction_info result) {
                return make_ready_future<stop_iteration>(stop_iteration::no);
            });
        });
    });
