...
table::make_reader(schema_ptr s,
    ...
    if (cache_enabled() && !slice.options.contains(query::partition_slice::option::bypass_cache)) {
        readers.emplace_back(_cache.make_reader(s, permit, range, slice, pc, trace_state, fwd, fwd_mr));
    } else {
    ...
    auto comb_reader = make_restricted_combined_reader(s, std::move(permit), std::move(readers),
...
flat_mutation_reader table::make_streaming_reader(schema_ptr schema, const dht::partition_range& range,
    ...
    readers.emplace_back(make_sstable_reader(schema, permit, _sstables, range, slice, pc, trace_state, fwd, fwd_mr));
...
table::add_sstable_and_update_cache(sstables::shared_sstable sst) {
    if (_is_bootstrap_or_replace) {
        // cache invalidation is bypassed as new data only is being added
        return add_repaired_sstable(std::move(sst));
    }
...
void table::rebuild_statistics() {
    ...
    auto sets = boost::range::join(*_sstables->all(), *_repair_sstables->all());
    for (auto&& tab : boost::range::join(_sstables_compacted_but_not_deleted, std::move(sets))) {
...
size_t table::sstables_count() const {
    return _sstables->all()->size() + _repair_sstables->all()->size();
}

std::vector<uint64_t> table::sstable_count_per_level() const {
    std::vector<uint64_t> count_per_level;
    for (auto&& sst : boost::range::join(*_sstables->all(), *_repair_sstables->all())) {
...
std::vector<sstables::shared_sstable> table::select_sstables(const dht::partition_range& range) const {
    ...
    if (auto ssts = _repair_sstables->select(range); ssts.size()) {
...
table::sstables_as_snapshot_source() {
    return snapshot_source([this] () {
        ...
        return mutation_source([this, sst_set, repair_sst_set] (schema_ptr s,
            ...
            return make_restricted_combined_reader(s, std::move(permit), std::move(readers), r,
            ...
            return [main_checker = make_partition_presence_checker(std::move(sst_set)),
                    repair_checker = make_partition_presence_checker(std::move(repair_sst_set))]
                ...
                if (main_checker(key) == partition_presence_checker_result::maybe_exists ||
                        repair_checker(key) == partition_presence_checker_result::maybe_exists) {
...
future<db::replay_position> table::discard_sstables(db_clock::time_point truncated_at) {
    ...
    struct pruner {
        ...
        void prune(db_clock::time_point truncated_at) {
            ...
            auto repair_pruned = cf.make_repair_sstable_set();
            ...
            prune(repair_pruned, cf._repair_sstables->all());

            cf._sstables = std::move(pruned);
            cf._repair_sstables = std::move(repair_pruned);
        }
    };
...
table::make_reader_excluding_sstables(schema_ptr s,
    ...
    readers.emplace_back(make_sstable_reader(s, permit, std::move(effective_repair_sstables), range, slice, pc, trace_state, fwd, fwd_mr));
    return make_restricted_combined_reader(s, std::move(permit), std::move(readers), range, slice, pc, trace_state, fwd, fwd_mr);
}
                ...
                return sst->move_to_new_dir(dir(), sst->generation(), false).then_wrapped([this, sst, &dirs_to_sync] (future<> f) {
                    if (!f.failed()) {
                        _sstables_staging.erase(sst->generation());
                         // Staging SSTable is only added to tracker if it isn't being processed by off-strategy.
                        if (!_repair_sstables->all()->contains(sst)) {
