...
void table::add_maintenance_sstable(sstables::shared_sstable sst) {
    // allow in-progress reads to continue using old list
    auto new_sstables = make_lw_shared<sstables::sstable_set>(*_maintenance_sstables);
    new_sstables->insert(sst);
    if (sst->requires_view_building()) {
        _sstables_staging.emplace(sst->generation(), sst);
    }
    _maintenance_sstables = std::move(new_sstables);
...
void table::rebuild_statistics() {
    ...
    auto sets = boost::range::join(*_sstables->all(), *_maintenance_sstables->all());
...
future<> table::run_offstrategy_compaction() {
    ...
    auto f = seastar::try_with_gate(_sstable_deletion_gate, [this, sstables_to_remove = old_sstables] {
       return with_semaphore(_sstable_deletion_sem, 1, [sstables_to_remove = std::move(sstables_to_remove)] {
           return sstables::delete_atomically(std::move(sstables_to_remove));
       });
    });
...
size_t table::sstables_count() const {
    return _sstables->all()->size() + _maintenance_sstables->all()->size();
}

std::vector<uint64_t> table::sstable_count_per_level() const {
    std::vector<uint64_t> count_per_level;
    for (auto&& sst : boost::range::join(*_sstables->all(), *_maintenance_sstables->all())) {
...
future<> table::snapshot(database& db, sstring name) {
    return flush().then([this, &db, name = std::move(name)]() {
       return with_semaphore(_sstable_deletion_sem, 1, [this, &db, name = std::move(name)]() {
        ...
        tables.reserve(all.size() + _maintenance_sstables->all()->size());
        for (auto& sst : boost::range::join(all, *_maintenance_sstables->all())) {
