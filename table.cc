...
table::make_sstables_mutation_source(std::vector<lw_shared_ptr<sstables::sstable_set>> sets) const {
    ...
    auto range_scanning_ms = [this] (lw_shared_ptr<sstables::sstable_set> sstables) -> mutation_source {
...
table::make_sstable_reader(schema_ptr s,
    ...
    auto ms = [&] () -> mutation_source {
        if (pr.is_singular() && pr.start()->value().has_key()) {
            ...
            assert(ssts_mutation_source.single_key);
            return *ssts_mutation_source.single_key;
        } else {
            assert(ssts_mutation_source.range_scanning);
            return *ssts_mutation_source.range_scanning;
        }
    }();
...
table::make_reader(schema_ptr s,
    ...
    if (cache_enabled() && !slice.options.contains(query::partition_slice::option::bypass_cache)) {
        ...
        readers.emplace_back(make_sstable_reader(s, permit, _sstables_mutation_source, range, slice, pc, std::move(trace_state), fwd, fwd_mr));
    }
...
flat_mutation_reader table::make_streaming_reader(schema_ptr schema, const dht::partition_range& range,
    ...
    readers.emplace_back(make_sstable_reader(schema, permit, _sstables_mutation_source, range, slice, pc, std::move(trace_state), fwd, fwd_mr));
...
void table::do_add_sstable(lw_shared_ptr<sstables::sstable_set>& sstables, sstables::shared_sstable sstable,
    ...
    } else if (enable_backlog_tracker == sstables::enable_backlog_tracker::yes) {
    ...
    refresh_sstables_mutation_source();
    update_stats_for_new_sstable(sstable->bytes_on_disk());
}

void table::add_sstable(sstables::shared_sstable sstable) {
    do_add_sstable(_sstables, std::move(sstable), sstables::enable_backlog_tracker::yes);
}
...
void table::add_maintenance_sstable(sstables::shared_sstable sst) {
    do_add_sstable(_maintenance_sstables, std::move(sst), sstables::enable_backlog_tracker::no);
}

future<>
table::add_sstable_and_update_cache(sstables::shared_sstable sst, sstables::enable_offstrategy offstrategy) {
    return get_row_cache().invalidate(row_cache::external_updater([this, sst, offstrategy] () noexcept {
        // FIXME: this is not really noexcept, but we need to provide strong exception guarantees.
        // atomically load all opened sstables into column family.
        if (offstrategy == sstables::enable_offstrategy::no) {
            add_sstable(sst);
            trigger_compaction();
        } else {
...
future<>
table::move_reshaped_sstables_into_main_set(const std::vector<sstables::shared_sstable>& old_sstables,
                                            const std::vector<sstables::shared_sstable>& new_sstables) {
    auto current_maintenance = _maintenance_sstables;
    auto new_maintenance_set = make_maintenance_sstable_set();

    std::unordered_set<sstables::shared_sstable> old(old_sstables.begin(), old_sstables.end());
    for (auto& tab : *current_maintenance->all()) {
        if (!old.contains(tab)) {
            new_maintenance_set->insert(tab);
        }
        co_await make_ready_future<>(); // yield if needed.
    }

    auto current_main = _sstables;
    auto new_main_set = _compaction_strategy.make_sstable_set(_schema);

    // this might seem dangerous, but "move" here just avoids constness
    for (auto& tab : boost::range::join(new_sstables, std::move(*current_main->all()))) {
        new_main_set.insert(tab);
        co_await make_ready_future<>(); // yield if needed.
    }

    _sstables = std::move(new_main_set);
    _maintenance_sstables = std::move(*new_maintenance_set);
    refresh_sstables_mutation_source();
    _cache.refresh_snapshot();
}
...
table::on_compaction_completion(sstables::compaction_completion_desc& desc) {
    ...
    class sstable_list_updater : public row_cache::external_updater_impl {
        ...
        sstables::sstables_mutation_source _sstables_mutation_source;
    public:
        ...
        virtual void execute() override {
            ...
            _t._sstables_mutation_source = std::move(_sstables_mutation_source);
        }
...
future<> table::run_offstrategy_compaction() {
    ...
    std::vector<sstables::shared_sstable> new_sstables, sstables_to_remove;
    ...
    co_await move_reshaped_sstables_into_main_set(old_sstables, new_sstables);

    auto f = seastar::try_with_gate(_sstable_deletion_gate, [this, sstables_to_remove = std::move(sstables_to_remove)] {
       return with_semaphore(_sstable_deletion_sem, 1, [sstables_to_remove = std::move(sstables_to_remove)] {
           return sstables::delete_atomically(std::move(sstables_to_remove));
       });
    });
    try {
        ...
        tlogger.error("Deletion of reshaped SSTables failed, due to {}. Ignored.", std::current_exception());
    }
...
future<> table::snapshot(database& db, sstring name) {
    return flush().then([this, &db, name = std::move(name)]() {
       return with_semaphore(_sstable_deletion_sem, 1, [this, &db, name = std::move(name)]() {
        ...
        tables.reserve(_all_sstables.sstables_size());
        _all_sstables.for_each_sstable([&tables] (const sstables::shared_sstable& sst) mutable {
