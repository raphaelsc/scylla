...
future<sstables::entry_descriptor> column_family::probe_file(sstring sstdir, sstring fname) {
    ...
    auto fut = sstable::ignore_if_irrelevant(*_schema, _schema->ks_name(), _schema->cf_name(), sstdir, comps.generation, comps.version, comps.format);
    return std::move(fut).then([this, sstdir = std::move(sstdir), comps] (bool relevant_to_current_shard) {
        if (!relevant_to_current_shard) {
            return make_ready_future<>();
        }
...
column_family::load_new_sstables(std::vector<sstables::entry_descriptor> new_tables) {
    return parallel_for_each(new_tables, [this] (auto comps) {
        ...
        return sst->load().then([this, sst] {
            ...
            if (ignore_sstable_if_irrelevant(*_schema, *sst)) {
                // Add sstable if relevant to current shard.
