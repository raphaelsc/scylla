...
future<> distributed_loader::open_sstable(distributed<database>& db, sstables::entry_descriptor comps,
    ...
    // shards. S is decided using formula `generation % smp::count` for
    // distributing evenly the resource usage among all shards.

    return db.invoke_on(comps.generation % smp::count, [&db, comps = std::move(comps), func = std::move(func)] (database& local) {
        ...
        return f.then([&db, comps = std::move(comps), func = std::move(func)] (sstables::sstable_open_info info) {
            return invoke_all_with_ptr(db, std::move(info.components),
                    [owners = std::move(info.owners), data = info.data, index = info.index, comps, func = std::move(func)] (database& db, auto components) {
                auto& cf = db.find_column_family(comps.ks, comps.cf);
                return func(cf, sstables::foreign_sstable_open_info(owners, std::move(components), data, index));
            });
        });
    });
}
...
future<> distributed_loader::populate_keyspace(distributed<database>& db, sstring datadir, sstring ks_name) {
    return seastar::async([&db, datadir = std::move(datadir), ks_name = std::move(ks_name)] {
        auto ksdir = datadir + "/" + ks_name;
        auto keyspaces = db.invoke_on(0, [] (auto& db) { return &db.get_keyspaces(); }).get0();
        ...
        auto& ks = i->second;
        auto&& cfs = db.invoke_on(0, [] (database& db) { return db.get_column_families(); }).get0();

        parallel_for_each(ks.metadata()->cf_meta_data() | boost::adaptors::map_values, [ks_name = std::move(ks_name), &ks, &cfs, &db] (schema_ptr s) mutable {
            utils::UUID uuid = s->id();
            lw_shared_ptr<column_family> cf = cfs[uuid];
...
// TODO: maybe kill this function by using database::get_non_system_keyspaces() in init_non_system_keyspaces().
...
future<> distributed_loader::init_system_keyspace(distributed<database>& db) {
    return seastar::async([&db] {
        ...
        const auto& cfg = db.invoke_on(0, [] (auto& db) { return db.get_config(); }).get0();
        ...
        auto make_ks_ready = [] (database& db) {
            return db.init_commitlog().then([&db] () mutable {
                auto& ks = db.find_keyspace(db::system_keyspace::NAME);
                return parallel_for_each(ks.metadata()->cf_meta_data(), [&db] (auto& pair) {
