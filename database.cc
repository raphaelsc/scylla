...
// Outer vector stores an entry for each clustering key range specified by the filter.
// Inner vector stores component ranges of the same clustering key, index 0 referring
// to component range 0, and so on.
using clustering_key_filter_ranges = std::vector<std::vector<nonwrapping_range<bytes_view>>>;
...
static clustering_key_filter_ranges
ranges_for_clustering_key_filter(const schema_ptr& schema, std::vector<nonwrapping_range<clustering_key_prefix>>& ck_filtering_all_ranges) {
    clustering_key_filter_ranges ranges;
...
static inline bool
contains_rows(const sstables::sstable& sst, const schema_ptr& schema, const clustering_key_filter_ranges& ranges) {
    ...
    return boost::algorithm::any_of(ranges, [&] (auto& range) {
