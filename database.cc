...
static std::vector<std::vector<nonwrapping_range<bytes_view>>>
ranges_for_clustering_range_filter(const schema_ptr& schema, std::vector<nonwrapping_range<clustering_key_prefix>>& ck_filtering_all_ranges) {
    std::vector<std::vector<nonwrapping_range<bytes_view>>> ranges;

    for (auto& r : ck_filtering_all_ranges) {
        // this vector stores a range for each component of a key, only one if not composite.
        std::vector<nonwrapping_range<bytes_view>> composite_ranges;
        ...
        while (start_it != start.end() && end_it != end.end()) {
            composite_ranges.push_back(nonwrapping_range<bytes_view>({{ std::move(*start_it++) }}, {{ std::move(*end_it++) }}));
        }
        while (start_it != start.end()) {
            composite_ranges.push_back(nonwrapping_range<bytes_view>({{ std::move(*start_it++) }}, {}));
        }
        while (end_it != end.end()) {
            composite_ranges.push_back(nonwrapping_range<bytes_view>({}, {{ std::move(*end_it++) }}));
        }
        // Inclusiveness is only propagated to the last element. We do that by rewriting last
        // range with bounds that use the correct inclusiveness.
        // FIXME: avoid this extra step by predicting the last range for component and propagating
        // its inclusiveness.
        auto& back = composite_ranges.back();
        stdx::optional<nonwrapping_range<bytes_view>::bound> start_bound, end_bound;
        if (back.start()) {
            start_bound = {{ std::move(back.start()->value()), r.start()->is_inclusive() }};
        }
        if (back.end()) {
            end_bound = {{ std::move(back.end()->value()), r.end()->is_inclusive() }};
        }
        back = { std::move(start_bound), std::move(end_bound) };
...
static inline bool
contains_rows(const sstables::sstable& sst, const schema_ptr& schema, const std::vector<std::vector<nonwrapping_range<bytes_view>>>& ranges) {
...
sstable_reader_filter(const std::vector<sstables::shared_sstable>& sstables, column_family& cf, const schema_ptr& schema,
    ...
    stats->sstables_checked_by_clustering_filter += candidates.size();
    ...
    if (ck_filtering_all_ranges.size() == 1 && ck_filtering_all_ranges[0].is_full()) {
        ...
        stats->surviving_sstables_after_clustering_filter += candidates.size();
        return candidates;
    }
    auto ranges = ranges_for_clustering_range_filter(schema, ck_filtering_all_ranges);
    if (ranges.empty()) {
        return {};
    }
    ...
    stats->surviving_sstables_after_clustering_filter += candidates_with_clustering_range.size();
...
column_family::make_sstable_reader(schema_ptr s,
    ...
    if (pr.is_singular() && pr.start()->value().has_key()) {
        ...
        return restrict_reader(make_mutation_reader<single_key_sstable_reader>(const_cast<column_family*>(this), std::move(s), _sstables, *pos.key(), ck_filtering, pc));
    } else {
...
database::setup_collectd() {
    ...
    _collectd.push_back(
        scollectd::add_polled_metric(scollectd::type_instance_id("database"
                , scollectd::per_cpu_plugin_instance
                , "queue_length", "clustering_filter")
                , scollectd::make_typed(scollectd::data_type::GAUGE, _cf_stats.clustering_filter_count)
    ));
    ...
    _collectd.push_back(
        scollectd::add_polled_metric(scollectd::type_instance_id("database"
                , scollectd::per_cpu_plugin_instance
                , "queue_length", "clustering_filter")
                , scollectd::make_typed(scollectd::data_type::GAUGE, _cf_stats.sstables_checked_by_clustering_filter)
    ));
    ...
    _collectd.push_back(
        scollectd::add_polled_metric(scollectd::type_instance_id("database"
                , scollectd::per_cpu_plugin_instance
                , "queue_length", "clustering_filter")
                , scollectd::make_typed(scollectd::data_type::GAUGE, _cf_stats.clustering_filter_fast_path_count)
    ));
    ...
    _collectd.push_back(
        scollectd::add_polled_metric(scollectd::type_instance_id("database"
                , scollectd::per_cpu_plugin_instance
                , "queue_length", "clustering_filter")
                , scollectd::make_typed(scollectd::data_type::GAUGE, _cf_stats.surviving_sstables_after_clustering_filter)
    ));
