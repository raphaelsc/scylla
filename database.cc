...
future<> column_family::replace_shared_sstable_by_unshared(int64_t shared_sstable_gen, int64_t unshared_sstable_gen) {
    ...
    auto it = boost::find_if(*sstables, [shared_sstable_gen] (auto& sst) { return sst->generation() == shared_sstable_gen; });
    if (it == sstables->end()) {
    ...
    return db.invoke_on_all([&db, ks_name = std::move(ks_name), cf_name = std::move(cf_name)] (database& db_local) {
        ...
        do_for_each(w->sstables, [w] (auto& sst) {
            ...
            try {
                ...
                dblog.info("resharding was abruptly stopped: {}", e.what());
            } catch (...) {
...
future<> distributed_loader::load_new_sstables(distributed<database>& db, sstring ks, sstring cf, std::vector<sstables::entry_descriptor> new_tables) {
    return parallel_for_each(new_tables, [&db] (auto comps) {
        ...
        return db.invoke_on_all([ks = std::move(ks), cfname = std::move(cf)] (database& db) {
            ...
            // TODO: call distributed_loader::reshard()
        });
    });
}
