...
public:
    explicit incremental_reader_selector(schema_ptr s,
        ...
        , _fn(std::move(fn)) {
        _s = std::move(s);
        _selector_position = _pr->start() ? _pr->start()->value() : dht::ring_position::min();
    ...
    virtual std::vector<flat_mutation_reader> create_new_readers(const dht::token* const t) override {
        ...
        // No need to forward position info when token T is a nullptr because that means reader selector
        // is being initialized and the lower bound of the first sstable_set::incremental_reader_selector's
        // interval is guaranteed to be inclusive.
