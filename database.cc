...
keyspace::make_column_family_config(const schema& s) const {
    ...
    cfg.read_concurrency_config = _config.read_concurreny_config;
...
database::make_keyspace_config(const keyspace_metadata& ksm) {
    ...
    cfg.read_concurreny_config.sem = &_read_concurrency_sem;
    cfg.read_concurreny_config.timeout = _cfg->read_request_timeout_in_ms() * 1ms;
    // Assume a queued read takes up 1kB of memory, and allow 2% of memory to be filled up with such reads.
    cfg.read_concurreny_config.max_queue_length = memory::stats().total_memory() * 0.02 / 1000;
