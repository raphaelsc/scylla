...
// them: a directory of the data dir, or a bucket of an object store. Which of
// those it is, and how the sstables in it are enumerated and opened, is storage
// specific, so it is left to sstable_directory.
std::vector<sstables::shared_sstable> load_sstables_of_table(schema_ptr schema, sstables::sstables_manager& sst_man,
    ...
    sstables::sstable_directory sst_dir(sst_man, schema, &schema->get_sharder(),
    ...
    sst_dir.process_sstable_dir(sstables::sstable_directory::process_flags::read_only()).get();
...
// The identity of the node whose data directory is being examined. Only
// operations which look sstables up by the node owning them need it, so it is
// worked out when one first asks, and a data dir which doesn't identify its
// node falls back to a made up identity, as every operation did before.
...
$ scylla sstable validate /path/to/md-123456-big-Data.db /path/to/md-123457-big-Data.db
    ...
    return app.run_async(argc, argv, [&app] (const operation& operation, const bpo::variables_map& raw_app_config) {
        ...
        sstables::sstables_manager sst_man(
            ...
            tracker,
            dir_sem,
            [&app_config, &dbcfg, permit, host_id = std::optional<locator::host_id>()] () mutable {
                if (!host_id) {
                    host_id = resolve_local_host_id(app_config, dbcfg, permit);
                }
                return *host_id;
            },
            *scf,
