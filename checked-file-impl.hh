...
public:
    ...
    checked_file_impl(io_check_error_handler error_handler, disk_error_signal_type& s, file f)
            : _error_handler(std::move(error_handler)), _signal(s) , _file(f) {
    ...
    virtual future<size_t> write_dma(uint64_t pos, const void* buffer, size_t len, const io_priority_class& pc) override {
        return do_io_check(_error_handler, _signal, [&] {
...
private:
    io_check_error_handler _error_handler;
    disk_error_signal_type &_signal;
    file _file;
};

inline file make_checked_file(io_check_error_handler error_handler, disk_error_signal_type& signal, file& f)
{
    return file(::make_shared<checked_file_impl>(std::move(error_handler), signal, f));
}
...
{
    return do_io_check(error_handler, signal, [&, error_handler] {
        return open_file_dma(name, flags, options).then([&, error_handler = std::move(error_handler)] (file f) {
            return make_ready_future<file>(make_checked_file(std::move(error_handler), signal, f));
        });
    });
}
