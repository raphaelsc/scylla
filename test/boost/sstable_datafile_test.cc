...
SEASTAR_TEST_CASE(test_backlog_controller) {
    return test_env::do_with_async([tmp = tmpdir()] (test_env& env) {
        /////////////
        // settings
        ...
        auto manager = std::make_unique<compaction_manager>(default_scheduling_group(), default_priority_class(), available_memory, as);

        auto add_sstable = [&env, &manager, gen = make_lw_shared<unsigned>(1)] (table& t, uint64_t data_size) {
        ...
        auto create_table = [&] () {
            ...
            cfg.enable_disk_writes = true;
            cfg.enable_cache = false;
            auto t = make_lw_shared<table>(s, cfg, column_family::no_commitlog(), *manager, cl_stats, *tracker);
        ...
        for (auto target_table_count : target_table_count_s) {
            ...
            std::vector<lw_shared_ptr<table>> tables;
            ...
            for (uint64_t t_idx = 0, available_space = all_tables_disk_usage; available_space >= estimated_flush_size; t_idx++) {
                ...
                testlog.info("T{}: {} tiers, with total size={}", t_idx, tiers, sstables::pretty_printed_data_size(table_size));
                tables.push_back(t);
                tables_total_size += table_size;
            }
            testlog.info("Created {} tables, with total size={}", tables.size(), sstables::pretty_printed_data_size(tables_total_size));
            results.push_back(result{ tables.size(), per_table_max_disk_usage, manager->get_compaction_controller().get_shares() });
        }
        for (auto& r : results) {
            testlog.info("Tables={} with max size={} -> Shares={}", r.table_count, sstables::pretty_printed_data_size(r.per_table_max_disk_usage), r.shares);
        }
    });
}
