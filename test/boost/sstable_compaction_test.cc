...
class table_state_for_test : public table_state_impl {
    column_family_for_tests& _t;
    bool _has_ongoing_compaction;
public:
    explicit table_state_for_test(column_family_for_tests& t, bool has_ongoing_compaction)
        : _t(t)
...
static table_state make_table_state_for_test(column_family_for_tests t, bool has_ongoing_compaction = false) {
    return table_state(::make_shared<table_state_for_test>(t, has_ongoing_compaction));
}
...
static future<std::vector<unsigned long>> compact_sstables(test_env& env, sstring tmpdir_path, std::vector<unsigned long> generations_to_compact,
    ...
    return f.then([&env, generations, sstables, s, create_sstables, min_sstable_size, tmpdir_path] () mutable {
        ...
        if (strategy == compaction_strategy_type::size_tiered) {
            ...
            leveled_manifest manifest = leveled_manifest::create(make_table_state_for_test(cf), candidates, 1, stcs_options);
...
SEASTAR_TEST_CASE(leveled_01) {
  return test_env::do_with([] (test_env& env) {
    ...
    auto candidates = get_candidates_for_leveled_strategy(*cf);
    sstables::size_tiered_compaction_strategy_options stcs_options;
    leveled_manifest manifest = leveled_manifest::create(make_table_state_for_test(cf), candidates, max_sstable_size_in_mb, stcs_options);
...
SEASTAR_TEST_CASE(leveled_stcs_on_L0) {
  return test_env::do_with([] (test_env& env) {
    ...
    {
        leveled_manifest manifest = leveled_manifest::create(make_table_state_for_test(cf), candidates, sstable_max_size_in_mb, stcs_options);
...
SEASTAR_TEST_CASE(basic_date_tiered_strategy_test) {
  return test_env::do_with([] (test_env& env) {
    ...
    auto sstables = manifest.get_next_sstables(make_table_state_for_test(cf), candidates, gc_before);
...
SEASTAR_TEST_CASE(time_window_strategy_correctness_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        table_state table_s = make_table_state_for_test(column_family_for_tests(env.manager(), s));
        ...
        auto new_bucket = twcs.newest_bucket(table_s, buckets, 4, 32,
        ...
        new_bucket = twcs.newest_bucket(table_s, buckets, 2, 32,
            time_window_compaction_strategy::get_window_lower_bound(duration_cast<seconds>(hours(1)), now));
        ...
        now = api::timestamp_clock::now().time_since_epoch().count();
        new_bucket = twcs.newest_bucket(table_s, buckets, 4, 32,
            time_window_compaction_strategy::get_window_lower_bound(duration_cast<seconds>(hours(1)), now));
...
SEASTAR_TEST_CASE(time_window_strategy_size_tiered_behavior_correctness) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        table_state table_s = make_table_state_for_test(cf);

        // past window cannot be compacted because it has a single SSTable
        BOOST_REQUIRE(twcs.newest_bucket(table_s, buckets, min_threshold, max_threshold, now).size() == 0);
        ...
        BOOST_REQUIRE(twcs.newest_bucket(table_s, buckets, min_threshold, max_threshold, now).size() == 2);
        ...
        BOOST_REQUIRE(twcs.newest_bucket(table_s, buckets, min_threshold, max_threshold, now).size() == size_t(min_threshold));
    });
}
...
SEASTAR_TEST_CASE(size_tiered_beyond_max_threshold_test) {
  return test_env::do_with([] (test_env& env) {
    ...
    auto desc = cs.get_sstables_for_compaction(make_table_state_for_test(cf), std::move(candidates));
...
SEASTAR_TEST_CASE(sstable_expired_data_ratio) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto descriptor = cs.get_sstables_for_compaction(make_table_state_for_test(cf), { sst });
        ...
        descriptor = cs.get_sstables_for_compaction(make_table_state_for_test(cf), { sst });
        BOOST_REQUIRE(descriptor.sstables.size() == 1);
        BOOST_REQUIRE(descriptor.sstables.front() == sst);
...
SEASTAR_TEST_CASE(sstable_run_based_compaction_test) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto do_compaction = [&] (size_t expected_input, size_t expected_output) -> std::vector<shared_sstable> {
            ...
            auto desc = cs.get_sstables_for_compaction(make_table_state_for_test(cf), std::move(input_ssts));
...
SEASTAR_TEST_CASE(compaction_strategy_aware_major_compaction_test) {
    return test_env::do_with_async([] (test_env& env) {
...

        {
            ...
            auto descriptor = cs.get_major_compaction_job(make_table_state_for_test(cf), candidates);
