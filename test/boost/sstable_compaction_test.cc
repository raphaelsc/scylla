...
public:
    explicit strategy_control_for_test(bool has_ongoing_compaction) : _has_ongoing_compaction(has_ongoing_compaction) {}
...
static std::unique_ptr<strategy_control> make_strategy_control_for_test(bool has_ongoing_compaction = false) {
...
SEASTAR_TEST_CASE(time_window_strategy_correctness_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto close_cf = deferred_stop(cf);
        auto table_s = make_table_state_for_test(cf, env);
        auto control = make_strategy_control_for_test();
...
SEASTAR_TEST_CASE(size_tiered_beyond_max_threshold_test) {
  return test_env::do_with([] (test_env& env) {
    ...
    auto table_s = make_table_state_for_test(cf, env);
    auto strategy_c = make_strategy_control_for_test();
