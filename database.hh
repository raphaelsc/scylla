...
// Range sstable reader that will not filter out mutation that doesn't belong to current shard.
mutation_reader make_nonlocal_shard_sstable_reader(schema_ptr s,
        lw_shared_ptr<sstables::sstable_set> sstables,
        const dht::partition_range& pr,
        const query::partition_slice& slice,
        const io_priority_class& pc,
