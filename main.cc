  ...
  try {
    ...
    return app.run_deprecated(ac, av, [&] {
        ...
        return seastar::async([cfg, &db, &qp, &proxy, &mm, &ctx, &opts, &dirs, &pctx, &prometheus_server, &return_value] {
            ...
            smp::submit_to(0, [&db] { return distributed_loader::init_system_keyspace(db); }).get();
            ...
            supervisor_notify("loading sstables");
            smp::submit_to(0, [&db, &proxy] { return distributed_loader::init_non_system_keyspaces(db, proxy); }).get();
