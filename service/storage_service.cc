...
void storage_service::run_bootstrap_ops() {
    ...
    // FIXME: conditionally use streaming alternative based on configuration value.
    return;
...
future<sstable_list_cmd_response> storage_service::sstable_list_cmd_handler(gms::inet_address coordinator, sstable_list_cmd_request req) {
    ...
    return _db.map_reduce0([req] (database& db) {
        return db.get_sstable_names(req.ks_name, req.cf_name);
    }, std::vector<sstring>(), concat<sstring>).then([] (const std::vector<sstring> res) {
        return sstable_list_cmd_response{ std::move(res) };
    });
}
