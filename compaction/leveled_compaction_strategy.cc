...
compaction_descriptor leveled_compaction_strategy::get_sstables_for_compaction(table_state table_s, std::vector<sstables::shared_sstable> candidates) {
    ...
    if (!candidate.sstables.empty()) {
        leveled_manifest::logger.debug("leveled: Compacting {} out of {} sstables", candidate.sstables.size(), table_s.sstable_set().all()->size());
        return candidate;
    }
    ...
    for (auto level = int(manifest.get_level_count()); level >= 0; level--) {
        ...
        return sstables::compaction_descriptor({ sst }, table_s.sstable_set(), service::get_local_compaction_priority(), sst->get_sstable_level());
    }
    return {};
}
