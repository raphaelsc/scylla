...
future<> compaction_manager::stop_ongoing_compactions(sstring reason, table* t) {
    auto ongoing_compactions = get_compactions().size();
    auto tasks = boost::copy_range<std::vector<lw_shared_ptr<task>>>(_tasks | boost::adaptors::filtered([t] (auto& task) {
        return task->compacting_table == t;
    }));
    cmlog.info("Stopping {} tasks for {} ongoing compactions for table {}.{} due to {}", tasks.size(), ongoing_compactions, t->schema()->ks_name(), t->schema()->cf_name(), reason);
    return stop_tasks(std::move(tasks), std::move(reason));
}
...
const std::vector<sstables::compaction_info> compaction_manager::get_compactions() const {
    auto to_info = [] (const lw_shared_ptr<task>& t) {
        ...
        ret.type = t->type;
        ret.ks_name = t->compacting_table->schema()->ks_name();
        ret.cf_name = t->compacting_table->schema()->cf_name();
