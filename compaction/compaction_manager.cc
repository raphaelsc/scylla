...
compaction_manager::run_with_compaction_disabled(table* t, std::function<future<> ()> func) {
    ++_compaction_state[t].compaction_disabled;
    return stop_ongoing_compactions("user-triggered operation", t).then(std::move(func)).finally([this, t] {
        if (_compaction_state.contains(t) && --_compaction_state[t].compaction_disabled == 0) {
            // we're turning it on again, use function that does not increment the counter further.
            submit(t);
        }
    });
}
...
future<> compaction_manager::stop_tasks(std::vector<lw_shared_ptr<task>> tasks, sstring reason) {
    // Set all tasks as stopping before deferring to prevent them from being postponed, etc.
    for (auto& t : tasks) {
        t->stopping = true;
    }
...
inline bool compaction_manager::can_proceed(const lw_shared_ptr<task>& task) {
    return (_state == state::enabled) && !task->stopping &&
        (task->type != sstables::compaction_type::Compaction || _compaction_state[task->compacting_cf].compaction_disabled == 0);
}
