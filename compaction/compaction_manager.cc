...
static lw_shared_ptr<sstables::compaction_info> create_compaction_info(column_family& cf, sstables::compaction_type type) {
    ...
    info->type = type;
    info->run_identifier = utils::UUID_gen::get_time_UUID(); // initialized here, but supposed to be overwritten later by compaction
    info->cf = &cf;
...
std::vector<sstables::shared_sstable> compaction_manager::get_candidates(const column_family& cf) {
    ...
    auto partial_run_identifiers = boost::copy_range<std::unordered_set<utils::UUID>>(get_compactions()
...
future<> compaction_manager::perform_sstable_scrub_validate_mode(column_family* cf) {
    return run_custom_job(cf, sstables::compaction_type::Scrub, [this, &cf = *cf, sstables = get_candidates(*cf)] (sstables::compaction_info& info) mutable -> future<> {
        class pending_tasks {
