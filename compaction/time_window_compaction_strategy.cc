...
time_window_compaction_strategy::get_sstables_for_compaction(table_state& table_s, std::vector<shared_sstable> candidates) {
    ...
    if (!expired.empty()) {
        ...
        return compaction_descriptor(has_only_fully_expired::yes, std::vector<shared_sstable>(expired.begin(), expired.end()), table_s.sstable_set(), service::get_local_compaction_priority());
    }

    auto compaction_candidates = get_next_non_expired_sstables(table_s, std::move(candidates), gc_before);
    return compaction_descriptor(std::move(compaction_candidates), table_s.sstable_set(), service::get_local_compaction_priority());
}
