...
void size_tiered_backlog_tracker::refresh_sstables_backlog_contribution() {
    ...
    auto threshold = _all.back()->get_schema()->min_compaction_threshold();

    for (auto& bucket : sstables::size_tiered_compaction_strategy::get_buckets(_all, _stcs_options)) {
        if (!sstables::size_tiered_compaction_strategy::is_bucket_interesting(bucket, threshold)) {
            continue;
        }
        _sstables_backlog_contribution += boost::accumulate(bucket | boost::adaptors::transformed([this] (auto& sst) -> double {
...
double size_tiered_backlog_tracker::backlog(const compaction_backlog_tracker::ongoing_writes& ow, const compaction_backlog_tracker::ongoing_compactions& oc) const {
    ...
    if (effective_backlog_bytes <= 0) {
        return 0;
    }
    if (total_backlog_bytes == 0) {
        return 0;
    }
...
void size_tiered_backlog_tracker::replace_sstables(std::vector<sstables::shared_sstable> old_ssts, std::vector<sstables::shared_sstable> new_ssts) {
    std::unordered_set<sstables::shared_sstable> old;
    for (auto& sst : old_ssts) {
        if (sst->data_size() > 0) {
            _total_bytes -= sst->data_size();
            old.insert(std::move(sst));
        }
    }
    _all.erase(boost::range::remove_if(_all, [&] (const sstables::shared_sstable& sst) {
        return old.contains(sst);
    }), _all.end());
    for (auto& sst : new_ssts) {
        if (sst->data_size() > 0) {
            _total_bytes += sst->data_size();
            _all.push_back(std::move(sst));
        }
    }
