...
static constexpr unsigned do_get_rpc_client_idx(messaging_verb verb) {
    ...
    case messaging_verb::SSTABLE_LIST_CMD:
...
void messaging_service::register_sstable_list_cmd(std::function<future<sstable_list_cmd_response> (const rpc::client_info& cinfo, sstable_list_cmd_request)>&& func) {
    register_handler(this, messaging_verb::SSTABLE_LIST_CMD, std::move(func));
}
future<> messaging_service::unregister_sstable_list_cmd() {
    return unregister_handler(messaging_verb::SSTABLE_LIST_CMD);
}
future<sstable_list_cmd_response> messaging_service::send_sstable_list_cmd(msg_addr id, sstable_list_cmd_request req) {
    return send_message<future<sstable_list_cmd_response>>(this, messaging_verb::SSTABLE_LIST_CMD, std::move(id), std::move(req));
}
